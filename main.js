...
function fn_Nadzat (v)                                // Получаем на вход функции двузначное число ...надцать
{
	...
	while ( v > 11 )
	{                                                     // Пока число больше 11-ти
	v = v - 1;                                            // Уменьшаем его на 1
	i ++;                                                 // Счетчик увеличиваем на 1
	}
	
	var s = document.getElementById("numstr").innerHTML = arr_Nadzat [ i ] ;    // Выводим элемент массива ... надцатых чисел ( 11 ... 19 )
...
function fn_Desiatki (v)                            // Получаем на вход функции двузначное число
{
	...
	i++; 			                                      // Счетчик увеличиваем на 1
		if ( v < 10 && v > 0 )
			i = i - 1;                                    // Уменьшаем индекс массива Десяток на 1, потому что без этого выдает следующий ( неправильный ) элемент массива
	}
	
	if ( v == 10 ) 
	{                                    // Если Число кратно 10-ти
		var  s2 = document.getElementById("numstr").innerHTML = arr_Desiatki [ i ] ;   // Выводим элемент массива Десятков
		return s2;
	}
	else 
	{
		var  s2 = document.getElementById("numstr").innerHTML = arr_Desiatki [ i ] + '  ' + fn_Edinizy ( v ) ;   // Выводим элемент массива Десятков и результат функции, вычисляющей Единицы
...
function fn_Sotni (v)                                   // Получаем на вход функции трехзначное число
{
	...
	var i = 0;                                           // Сбрасываем счетчик цикла
		...
		i++; 			                                 // Счетчик увеличиваем на 1
			if ( v < 100 && v > 0 )
				i = i - 1;                               // Уменьшаем индекс массива Сотен на 1, потому что без этого выдает следующий ( неправильный ) элемент массива
		}
		
		if (v == 100)
		{	                                 // Если Число - ровная сотня
			s3 = document.getElementById("numstr").innerHTML = arr_Sotni [ i ] ;                         // Выводим элемент массива Сотен
				return s3;
		}
		else if  (v > 0 && v <10)
		{                      // Если Число ( 101 ... 109 )
			s3 = document.getElementById("numstr").innerHTML = arr_Sotni [ i ] + '  ' + fn_Edinizy (v);  // Выводим элемент массива Сотен и Единиц
			return s3;
		}
		else if  ( v > 10 && v <20 ) 
		{                  // Если Число ( 111 ... 119 )
			s3 = document.getElementById("numstr").innerHTML = arr_Sotni [ i ] + '  ' + fn_Nadzat (v);   // Выводим элемент массива Сотен и Надцать
			return s3;
		}
		else
			s3 = document.getElementById("numstr").innerHTML = arr_Sotni [ i ] + '  ' + fn_Desiatki (v) ;   // Выводим элемент массива Сотен и результат функции, вычисляющей Десятки
			return s3; 
}
...
razryad = 0;
	...
	{                                                    //  Если число от 1 000  до  1 000 000,
	...
	else if  ( v > 999999 && v < 1000000000 ) 
	{                                          									//  Если число от 1 000 000  до  1 000 000 000, 
